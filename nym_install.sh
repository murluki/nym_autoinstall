...
if [ ! -f nym-mixnode ] || [ "$(./nym-mixnode | grep Nym | cut -c 13- )" != "$VERSION" ]
   then
...
## Full install, config and launch of the nym-mixnode
  if [ "$1" = "-i" ]; then
    ...
    if [ ! -e /home/nym/nym-mixnode ] ; then nym_download ; fi
    nym_chmod
    nym_chown
    nym_init
    nym_systemd_print
...
## Sign with user input 
  ...
  then
    ...
    # borrows a shell for nym user to initialize the node config.
    #set -x
    sudo -u nym -H /home/nym/nym-mixnode sign --id $directory --text ${telegram} 2>&1 | tee -a ${directory}_claim.txt && chown nym:nym ${directory}_claim.txt
    printf "%b\n\n\n"
  fi
